...
import { CalendarCheck, FileText, Target, MessageSquare, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#1e1e1e] shadow-sm rounded-lg p-4 border border-gray-100 dark:border-gray-800">
          ...
          <p className="text-2xl font-bold mt-1 text-ruet-blue dark:text-blue-400">{coAttained}<span className="text-sm font-normal text-gray-500">/{coEntries.length}</span></p>
        </div>
        <div className="bg-white dark:bg-[#1e1e1e] shadow-sm rounded-lg p-4 border border-gray-100 dark:border-gray-800">
          <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-medium">Course Type</p>
          <p className="text-2xl font-bold mt-1 text-gray-900 dark:text-white">{course.type}</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#1e1e1e] shadow rounded-lg p-6 border border-gray-100 dark:border-gray-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button onClick={() => onNavigate('attendance_info')} className="flex items-center justify-between p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors text-left group">
          ...
          <button onClick={() => onNavigate('marksheet')} className="flex items-center justify-between p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors text-left group">
            <div className="flex items-center">
              ...
              <div>
                ...
                <span className="text-xs text-indigo-600/70 dark:text-indigo-400/70">Assessment and total marks</span>
              </div>
            </div>
            <ChevronRight size={18} className="text-indigo-400 group-hover:translate-x-1 transition-transform" />
          </button>

          <button onClick={() => onNavigate('obe_attainment')} className="flex items-center justify-between p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/40 transition-colors text-left group">
            <div className="flex items-center">
              ...
              <div>
                ...
                <span className="text-xs text-green-600/70 dark:text-green-400/70">CO/PO attainment and KPI status</span>
              </div>
            </div>
            <ChevronRight size={18} className="text-green-400 group-hover:translate-x-1 transition-transform" />
          </button>

          <button onClick={() => onNavigate('give_feedback')} className="flex items-center justify-between p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors text-left group">
            <div className="flex items-center">
